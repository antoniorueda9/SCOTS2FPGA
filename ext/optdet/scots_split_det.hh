...
#define MAX_STACK_TRACE_LEN 100
                ...
                struct split_tool_params {
                    ...
                    int32_t m_ss_dim;
                    //If true then we need the per-input split BDDs
                    bool m_is_input;
                    //If true then we need the domain BDD
                    bool m_is_supp;
                };
                ...
                static SwitchArg * p_is_input = NULL;
                static SwitchArg * p_is_supp = NULL;
                
                /**
                 * This functions does nothing more but printing the program header information
                 */
                static void print_info() {
                    print_info("BDD Splitter for SCOTSv2.0");
                }
                ...
                void create_arguments_parser() {
                    ...
                    p_is_input = new SwitchArg("i", "input", string("Request the splitting ") +
                                               string("the controller per input"), *p_cmd_args, false);
                    
                    //Request the domain BDD, default is false
                    p_is_supp = new SwitchArg("p", "support", string("Request the reordered  ") +
                                              string("controller support BDD"), *p_cmd_args, false);
                ...
                static void extract_arguments(const uint argc,
                    ...
                    params.m_is_input = p_is_input->getValue();
                    LOG_USAGE << "The per-input splitting is: "
                    << (params.m_is_input ? "" : "NOT ") << "NEEDED" << END_LOG;

                    params.m_is_supp = p_is_supp->getValue();
                    LOG_USAGE << "The controller's domain is: "
                    << (params.m_is_supp ? "" : "NOT ") << "NEEDED" << END_LOG;
                    
                    ASSERT_CONDITION_THROW(!params.m_is_supp && !params.m_is_input,
                                           "Nothing to be done request domain or input splitting!");
                }
                ...
                void destroy_arguments_parser() {
                    ...
                    SAFE_DESTROY(p_is_input);
                    SAFE_DESTROY(p_is_supp);
